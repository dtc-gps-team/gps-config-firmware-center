 ...
 * (ยืนยันกับ kittiphong แล้วบน issue #26 — ดู schema.prisma comment; ชื่อ
 * `sw_approved` อ้างอิงถึง SW ตัวเดิมก่อนแยก role ในเชิงประวัติเท่านั้น)
 */
 ...
 * approved/synced ไปแล้ว)" — รวม `testing` ไว้ด้วยเพราะ ConfigEngineer ต้อง
 * ทดสอบซ้ำได้
...
/** สถานะที่ ConfigEngineer ปักผลตัดสินใจได้ (Stage 4, `decideConfig`) — ต้องเป็น `draft`
 ...
 * `rejectConfig`) — ต้องเป็น `testing` เท่านั้น (ConfigEngineer ต้อง `decide(passed:true)`
