...
| `/config/{configId}/versions` | GET | `listConfigVersions` | resource `config` action `Read` — เหมือน `getConfig` (SW/Operation/ST/OT/Auditor/Admin) · version history เป็นแค่ snapshot ของ `fields` ที่ Read config เห็นอยู่แล้ว + Auditor (compliance) ต้องดูได้ จึง**ไม่แยก resource** ต่างจาก `simulate`/`decide` (Stage 5, #26 ข้อ 9) |
...
| `/config/{configId}/deletion-requests/keep` | POST | `keepConfigDeletionRequest` | **SW** (resource `config` action `Update` เดิม · ผู้สร้าง/SW กด "เก็บไว้" ระหว่าง grace period → `cancelled` + reset นาฬิกา 90 วัน) — key ด้วย config id ไม่ใช่ request id (ผู้ใช้ได้ notification เรื่อง Config ไม่รู้ request id) |
