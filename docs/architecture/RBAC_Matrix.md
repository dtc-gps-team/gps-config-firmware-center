...
| `/notifications/device-tokens` | POST | `registerDeviceToken` | ทุก Role ที่ login แล้ว — ลงทะเบียน/อัปเดต FCM token ของเครื่องตัวเอง (upsert ตาม token, `userId` ผูกกับ JWT ไม่รับจาก client) · **JwtAuthGuard อย่างเดียว ไม่มี PermissionGuard/resource ใหม่** — self-scoped ล้วน ทุก Role ที่ login ผ่าน Mobile ต้องรับ push ของตัวเองได้ ไม่มี Role ไหนควรถูกห้าม (mirror `/notifications` GET · Section 5 ข้อ 8) — Push Notification groundwork #17 (ดู changelog แก้ครั้งที่ 18) |
