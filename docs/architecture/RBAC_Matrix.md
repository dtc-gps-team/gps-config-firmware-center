...
- **ตาราง join `CONFIG_DEFINITION_MODEL_SUPPORT` (ผูก `ConfigFieldDefinition` กับ deviceModel/protocol เฉพาะ) ยังไม่ได้ออกแบบ** (พบระหว่างทำ task #12 Config Definition Lookup โดย kittiphong): task #12 ปลดล็อกตาราง `ConfigFieldDefinition` + `GET /config-definitions` (catalog รวมทั้งระบบ) ไว้แล้ว แต่การจะเอา catalog ไป validate `fields` ของ Config จริง (ตอน import/simulate) ต้องรู้ว่า field ไหน "จำเป็น/ใช้ได้" กับ deviceModel/protocol ตัวไหน — ซึ่งต้องมีตาราง join แยก (`CONFIG_DEFINITION_MODEL_SUPPORT` หรือชื่ออื่น) ที่ **ยังไม่มีใน `schema.prisma` และยังไม่ได้ออกแบบ field/ความสัมพันธ์** — **paveekornk (A) เจ้าของ module `config`/`device-simulator` ยังไม่ได้รีวิว design ตารางนี้** ต้องคุยกับ A ก่อนสร้าง (อาจกระทบวิธี query ใน `ConfigService`/`DeviceSimulator` ที่ A ดูแล) — **task #12 จงใจไม่สร้างตาราง join นี้** ทำแค่ catalog อ่านอย่างเดียวก่อน
