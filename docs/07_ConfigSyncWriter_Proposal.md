...
## 2. ขอบเขต: ทำได้ตอนนี้ vs ต้องรอ handoff

| ส่วน | ทำได้เลย (ขอบเขตงานฝึกงาน) | ต้องรอข้อมูลจากทีม (handoff) |
|---|---|---|
...
| โหมด `docker` (ยิง TCP เข้า `127.0.0.1:801`) | | ต้องรู้รูปแบบคำสั่งเขียน + มีเครื่องทดสอบ |
| โหมด `production` (ยิงเข้า `config.dtc.co.th:909`) | | คำสั่งเขียน + สิทธิ์ + **คำสั่งเริ่มจากทีม** (ตอนนี้ชะลอไว้ v3.4) |

Checkpoint Sprint 2 กำหนดไว้แค่ *"อนุมัติ Config แล้วเห็น Log การเขียนเข้าระบบเดิม
(โหมด mock และ docker)"* และแผนอนุญาตให้ผ่านเฉพาะ mock ได้ถ้ายังไม่ได้คำตอบ TBD
(`02_GPS_Development_Plan.md` §หมายเหตุแถวที่ 9)
...
## 8. สิ่งที่ต้องได้จากทีม/พี่เลี้ยงก่อนทำ docker/production

(ดูลิสต์เต็มที่คุยกันแล้ว — สรุปหัวข้อ)
